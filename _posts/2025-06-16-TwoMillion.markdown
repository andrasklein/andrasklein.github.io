...
author: klein
image:
  path: /images/images-twomillion/image_38.png
  
---

Since I am starting out fresh I am going to be using the "Guided Mode" on this retired box. The Write Up is going to be tailored around the path and questions of how the "Guided Mode" goes through this box.
...
Let's go for the lower hanging fruit which in this case is port 80/http

By typing the IP address into the search bar we see that it gets redirected to twomillion.htb
...
The function discloses tha API endpoint.
...
By going to the Labs > Access page the Connection Pack button is goint to appear.
...
I sent the previously captured request to repeater and started to back with the directories untill I listed out the available endpoints.
...
The next step would be to change my account to have admin priviliges. For this I need to update my account with the previously mentioned */api/v1/admin/settings/update endpoint
...
With these credentials I can change to the admin user and found the user flag.
...
I found the following email's and passwords but this did not led me further.
...
This is going to list everything that belongs to the admin user expect run/proc/sys.
...
Based on the emial there is a serious CVE, let's search based on the *OverlayFS / FUSE hint.
...
After this I dowloaded with *wget the file to the target machine.
```
...
And in another terminal on the target box I ran the final command to get root.
...
And this let me to the root flag as well.
